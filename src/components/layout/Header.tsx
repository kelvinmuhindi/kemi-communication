          ...
          <ul className="flex gap-7">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="group relative py-1.5 text-sm font-semibold text-brand-ink-soft transition-colors hover:text-brand-orange-deep"
                >
                  {link.label}
                  <span className="absolute bottom-0 left-0 h-0.5 w-0 bg-brand-orange transition-all duration-200 group-hover:w-full" />
                </a>
              </li>
            ))}
          </ul>
